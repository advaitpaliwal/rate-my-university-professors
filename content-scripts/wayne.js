...
function getFirstAndLastName(fullName) {
  return [fullName.split(",")[1].split(" ")[1], fullName.split(", ")[0]];
}
...
async function fetchProfData(fullName) {
  ...
  const url =
    ...
    `https://www.ratemyprofessors.com/filter/professor/?page=1&filter=teacherlastname_sort_s+asc&query=${
      getFirstAndLastName(fullName)[0]
    }%20${
      getFirstAndLastName(fullName)[1]
    }&queryoption=TEACHER&queryBy=schoolId&sid=1150`;
...
async function updateElement(element, data, fullName) {
  let num_ratings = "0",
    prof_rating = "",
    prof_name = fullName,
    profUrl = `https://www.ratemyprofessors.com/search/teachers?query=${fullName}&sid=U2Nob29sLTExNTA=`;
  if (data) {
...
function changePage() {
  let target = document.getElementsByClassName("email");
  if (
  ...
  try {
    for (let i = 0; i < target.length; i++) {
      if (![""].includes(target[i].parentNode.className)) {
        return false;
      }
      const textElement = target[i].textContent.trim();
      if ([""].includes(textElement) || textElement.split(" ").length > 3) {
        continue;
      }
      target[i].style.textDecoration = "none";
      const fullName = textElement;
      if (fullName) {
        updateProfData(target[i], fullName);
      }
    }
    return true;
  } catch (err) {
